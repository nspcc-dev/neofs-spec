...
- Bearer token ACL rules in the request,
- Extended ACL rules in the FS chain smart contract.

\Glspl{acl} specify a set of actions that a particular user or a group of users can do with objects in the container. Each request coming through a storage node gets verified against those rules and rejected if the requests's action is not allowed.

### Basic ACL

Basic ACL is a part of the container structure, and it is always created simultaneously with the container. It is never subject to any changes. It is a 32-bit integer with a bit field in the following format:
...
| **S**  | System  | Originally intended for Inner Ring and/or container nodes in the current version of network map, currently ignored      |
...
Container nodes ignore these rules and can access any container data to
serve the container properly. There are also some container nodes-specific
actions like Replicate that are not allowed for any other entities.
...
Extended ACL is stored in the container smart contract in FS chain. This means it can be changed during container lifetime and there will be only one latest version of it in use. Only the container owner, or an entity authorized by owner with a session token, can change the Extended ACL rules. Since it is stored in a form of a stable serialized protobuf structure, eACL table can be only replaced with a new version, not altered or changed in-place in any way.

Extended ACL can only restrict whatever is allowed by basic ACL rules, but it can never ease the rules set by basic ACL. If something is denied at Basic ACL level, it can never be allowed again by eACL. If Basic ACL contains Allow, eACL may specify some rules to a finite list of allowed keys and Deny all others. If Basic ACL already contains Deny, eACL can do nothing. System role is not allowed for eACL rules.

When a user creates a container with the F-bit of Basic ACL set to 0, he doesn't need to settle the rules immediately. Unset (non-existent) Extended ACL is treated like a table with no rules, so only basic ACL rules are relevant in this case. However eACL table can also be set at the same time with container creation (same request and transaction).
...
| Target    | Subject's role class or a list of accounts to match                                |
